"""
Realistic LULD Halt Resumption Backtest — RVOL Filter
=======================================================
Strategy
--------
...
OUT_PNG        = Path("TESTING/halt_equity_curve_rvol.png")
OUT_CSV        = Path("TESTING/halt_trades_rvol.csv")
...
MAX_POSITION   = 20_000.0    # hard cap per position
SLIP_BPS_ENTRY = 50          # 50 bps slippage on entry (conservative — resumption gap)
SLIP_BPS_EXIT  = 25          # 25 bps slippage on exit (EOD, more liquid)

# IBKR tiered commission: $0.005/share, min $1.00, max 1% of trade value
def ibkr_commission(shares, trade_value):
    raw = abs(shares) * 0.005
    return max(1.0, min(raw, trade_value * 0.01))
...
# ── RVOL filter ───────────────────────────────────────────────────────────────
# RVOL = (vol/min in 5min before halt) / (vol/min from open to that point)
# Measures volume "surge" into the halt vs the stock's own pace earlier today.
# Set to 0 to disable.  Try 2.0, 3.0, 5.0 to see quality improve.
RVOL_WINDOW_MIN = 5          # minutes before halt_start to measure surge
MIN_RVOL        = 1.0        # minimum RVOL required to take the trade
...
for date, day_ev in ev.groupby("event_date"):
    ...
    positions      = {}    # key -> dict
    traded_symbols = set() # tickers already entered today — max 1 per ticker
    ...
    for _, row in day_ev.sort_values("anchor_ts_utc").iterrows():
        ...
        #
        #   RVOL = surge rate / baseline rate
        #
        #   surge_rate    = shares/min in the RVOL_WINDOW_MIN before halt_start
        #   baseline_rate = shares/min from market open to start of surge window
        #
        #   A stock doing 10x its own pace in the final 5 minutes before a halt
        #   shows real demand — "desire" to own it at any price.
        #   Events with no baseline bars (halted at/near open) are skipped.
        ...
        if baseline_bars.empty or surge_bars.empty:
            # no baseline to compare against — can't compute RVOL, skip
            continue
        ...
        baseline_rate  = baseline_vol / max(baseline_min, 1.0)   # shares/min
        surge_rate     = surge_vol    / RVOL_WINDOW_MIN           # shares/min
        ...
        if rvol < MIN_RVOL:
            continue   # not enough volume surge into the halt

        # ── 5. ticker dedup (disabled — multiple positions per ticker allowed) ──
        symbol = row["symbol_resolved"]

        # ── 5. market order simulation ────────────────────────────────────────
        ...
        # ── 5. position sizing ────────────────────────────────────────────────
        entry_exec = first_bar_open * (1.0 + SLIP_BPS_ENTRY / 10_000)
        if entry_exec < 1.0:
            continue

        pos_usd = min(
            equity   * POSITION_PCT,   # 5% of equity
            MAX_POSITION,              # hard cap
        ...
        entry_comm    = ibkr_commission(shares, pos_usd)
        equity       -= entry_comm     # deduct commission immediately
        available    -= pos_usd        # reduce deployable cash

        traded_symbols.add(symbol)
        key = f"{symbol}_{row['event_id']}"
        positions[key] = {
            "symbol":    symbol,
    ...
    for key, pos in positions.items():
        ...
        exit_exec  = exit_px_raw * (1.0 - SLIP_BPS_EXIT / 10_000)
        gross_pnl  = (exit_exec - pos["entry_px"]) * pos["shares"]
        exit_comm  = ibkr_commission(pos["shares"], exit_exec * pos["shares"])
        pnl        = gross_pnl - exit_comm
...
ax_eq.set_title(
    "LULD Halt Resumption — Stress Test  "
    r"(\$1k start | 5% sizing | \$20k cap | 50bps entry / 25bps exit slip | IBKR commissions)",
...
# Load the full CSV (which has rvol per trade) and sweep thresholds to show
# how win rate, median return, trade count, and max DD change with RVOL floor.

print("Building RVOL threshold sweep chart ...")
full_tr = pd.read_csv(OUT_CSV)

thresholds = [0, 1, 1.5, 2, 3, 4, 5, 7, 10, 15, 20]
sweep = []
for t in thresholds:
    sub = full_tr[full_tr["rvol"] >= t].copy()
    if len(sub) < 5:
        sweep.append({"rvol_min": t, "trades": len(sub),
                      "wr": None, "med_r": None, "avg_r": None, "dd": None})
        continue
    eq  = START_EQUITY + sub["pnl"].cumsum().values
    ...
    sweep.append({
        ...
        "med_r":    sub["ret_pct"].median(),
        "avg_r":    sub["ret_pct"].mean(),
        "dd":       dd,
    })

sw = pd.DataFrame(sweep)
valid = sw[sw["wr"].notna()]

fig2, axes2 = plt.subplots(2, 2, figsize=(14, 9), facecolor="white",
                            gridspec_kw={"hspace": 0.45, "wspace": 0.32})
fig2.suptitle("RVOL Threshold Sweep — How Filter Strictness Affects Strategy Quality",
              fontsize=13, fontweight="bold", color="#1E293B", y=0.98)

C_LINE = "#2563EB"

def sweep_plot(ax, y_col, ylabel, title, fmt="{:.1f}"):
    ax.plot(valid["rvol_min"], valid[y_col], color=C_LINE, lw=2, marker="o",
            markersize=5, zorder=3)
    ax.axvline(MIN_RVOL, color="#F59E0B", lw=1.5, ls="--", alpha=0.8,
               label=f"Current filter ({MIN_RVOL}x)")
    for _, row in valid.iterrows():
        ax.annotate(fmt.format(row[y_col]),
                    (row["rvol_min"], row[y_col]),
                    textcoords="offset points", xytext=(0, 7),
                    ha="center", fontsize=7.5, color="#475569")
    ax.set_xlabel("Min RVOL", fontsize=10)
    ax.set_ylabel(ylabel, fontsize=10, fontweight="bold")
    ax.set_title(title, fontsize=11, fontweight="bold", color="#1E293B")
    ax.legend(fontsize=8.5)
    ax.set_xticks(valid["rvol_min"])

sweep_plot(axes2[0,0], "trades",  "# Trades",        "Trade Count vs RVOL Floor", "{:.0f}")
sweep_plot(axes2[0,1], "wr",      "Win Rate (%)",     "Win Rate vs RVOL Floor",    "{:.1f}%")
sweep_plot(axes2[1,0], "med_r",   "Median Ret (%)",   "Median Trade Return vs RVOL Floor", "{:.1f}%")
sweep_plot(axes2[1,1], "dd",      "Max Drawdown (%)", "Max Drawdown vs RVOL Floor", "{:.1f}%")

OUT_SWEEP = Path("TESTING/rvol_sweep.png")
fig2.savefig(OUT_SWEEP, dpi=160, facecolor="white", bbox_inches="tight")
