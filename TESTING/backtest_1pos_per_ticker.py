...
OUT_PNG        = Path("TESTING/halt_equity_curve_1pos_per_ticker.png")
OUT_CSV        = Path("TESTING/halt_trades_1pos_per_ticker.csv")
...
for date, day_ev in ev.groupby("event_date"):
    ...
    for _, row in day_ev.sort_values("anchor_ts_utc").iterrows():
        ...
        # ── 4. max 1 position per ticker per day ─────────────────────────────
        symbol = row["symbol_resolved"]
        if symbol in traded_symbols:
            continue   # already entered this ticker today — skip repeat halt
